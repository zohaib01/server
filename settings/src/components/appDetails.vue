	...
	<div id="app-details-view" style="padding: 20px;">
		...
		<div class="actions">
			<div class="actions-buttons">
				...
				<input v-if="!app.active && app.isCompatible" class="enable primary" type="button" :value="enableButtonText" v-on:click="enable(app.id)" v-tooltip.auto="enableButtonTooltip" :disabled="!app.canInstall || installing || loading(app.id)" />
				<input v-else-if="!app.active" class="enable primary" type="button" :value="forceEnableButtonText" v-on:click="enable(app.id)" v-tooltip.auto="forceEnableButtonTooltip" :disabled="installing || loading(app.id)" />
			</div>
